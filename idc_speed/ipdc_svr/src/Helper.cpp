...
void InitConfig(const char *pszConfigFileName)
{
    ...
    pszTemp = GetConf("iProcessNum");
    if (pszTemp && atoi(pszTemp))
        g_svrConfig.iProcessNum = atoi(pszTemp);

    pszTemp = GetConf("iSyncIPItvlSec");
    if (pszTemp && atoi(pszTemp))
        g_svrConfig.iSyncIPItvlSec = atoi(pszTemp);

    //DB conf
    pszTemp = GetConf("DB_HOST");
    if (pszTemp)
        g_svrConfig.sDBHost = pszTemp;

    pszTemp = GetConf("DB_USER");
    if (pszTemp)
        g_svrConfig.sDBUser = pszTemp;

    pszTemp = GetConf("DB_PASS");
    if (pszTemp)
        g_svrConfig.sDBPass = pszTemp;
}
