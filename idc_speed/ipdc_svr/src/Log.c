        ...
        int rollCount)
{
    ...
    strncpy(pLog->log_name, fileName, PATH_LEN - 1);
    ...
    pLog->log_filecount = rollCount < 3 ? 3 : rollCount;
    pLog->log_std = 0;

    if (0 != pthread_mutex_init(&pLog->log_mutex, NULL))
        return NULL;
    ...
    {
        snprintf(curfile, PATH_LEN - 1, "%s", pLog->log_name);
