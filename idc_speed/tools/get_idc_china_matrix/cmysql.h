...
public:
	CMysql();
	CMysql(const char* szHost, const char* szUser, const char* szPass, unsigned short uPost = 0);
	~CMysql();

	int Close();
	int Connect(const char* szHost, const char* szUser, const char* szPass);
	int Connect(const char* pszSqlString = NULL );
	bool IfConnected(const char* szHost);

       void Safe_EscapeMysqlString(const char* pszSqlString , string &strSql );
...
private:
	...
	unsigned short m_uPort; //数据库端口
	string m_strLastDB; // 上一次选择的数据库名
