...
inline void GetValueUInt(unsigned &iValue, const char *pszName)
{
    char *pszTemp = GetConf(pszName);
    if (pszTemp != NULL)
        iValue = (unsigned) atoi(pszTemp);
}
...
void InitConfig(const char *pszConfigFileName)
{
    ...
    GetValueString(g_svrConfig.sIPDCSvrIP, "TaskSvrIP");
    GetValueUInt(g_svrConfig.uIPDCSvrPort, "TaskSvrPort");

    GetValueUInt(g_svrConfig.uIPAttrSyncTimeItvlSec, "IPAttrSyncTimeItvlSec");
    GetValueUInt(g_svrConfig.uDetectItvlSec, "DetectItvlSec");

    GetValueUInt(g_svrConfig.uMaxIPNumPerAttr, "MaxIPNumPerAttr");
    ...
    GetValueUInt(g_svrConfig.uMaxPingFileRemainDay, "max_ping_data_file_remain_day");
}
