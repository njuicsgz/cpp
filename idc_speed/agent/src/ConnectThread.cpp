...
{
    //循环执行，该线程不退出
    CIPDCApi stApi(g_svrConfig.sIPDCSvrIP.c_str(), g_svrConfig.uIPDCSvrPort);
    for(;;)
    {
        //登录服务器
        int iCount = 0;
        for(; iCount < 3; iCount++)
        {
            ...
            Err("[CConnectThread::Run]:Login to %s:%u failed", g_svrConfig.sIPDCSvrIP.c_str(), g_svrConfig.uIPDCSvrPort );
            stApi.Close();
            sleep(10);
        }
        ...
        {
            sleep(60);
            Err("[CConnectThread::Run]:Login to %s:%u failed 3 times", g_svrConfig.sIPDCSvrIP.c_str(), g_svrConfig.uIPDCSvrPort );
            continue;
        }

        Info("[CConnectThread::Run]:Login to %s:%u success", g_svrConfig.sIPDCSvrIP.c_str(), g_svrConfig.uIPDCSvrPort );
        ...
        while (true)
        {
            ...
            {
                Err("[CConnectThread::Run]:Heartbeat to %s:%u failed", g_svrConfig.sIPDCSvrIP.c_str(), g_svrConfig.uIPDCSvrPort );
                break;
            }

            //Info("[CConnectThread::Run]:Heartbeat to %s:%u success", g_svrConfig.sIPDCSvrIP.c_str(), g_svrConfig.uIPDCSvrPort );
            ...
            if (iRet == ERROR_FD_DOWN)
            {
                ...
                Err("[CConnectThread::Run]:Recv from %s:%u failed", g_svrConfig.sIPDCSvrIP.c_str(), g_svrConfig.uIPDCSvrPort );
                break;
            }
