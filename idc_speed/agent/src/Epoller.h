...
public:
    ...
    void add(int fd, int flag = EPOLLIN | EPOLLERR | EPOLLHUP)
            throw (std::runtime_error);
    void modify(int fd, int flag = EPOLLIN | EPOLLERR | EPOLLHUP | EPOLLOUT)
            throw (std::runtime_error);
...
public:
    ...
    CEPollResult(const CEPollResult& right) :
        _events(right._events), _size(right._size)
    {
    }
...
protected:
    CEPollResult(epoll_event* events, size_t size) :
        _events(events), _size(size)
    {
    }
...
public:
    ...
    protected:
        iterator(size_t index, CEPollResult& res) :
...
inline void CEPoller::create(size_t iMaxFD) throw (std::runtime_error)
{
    ...
    if (_fd == -1)
        throw std::runtime_error("epoll_create fail [" + std::string(strerror(
                errno)) + "]");
...
inline CEPollResult CEPoller::wait(int iTimeout) throw (std::runtime_error)
{
    ...
    {
        if (errno != EINTR)
            throw std::runtime_error("epoll_wait fail [" + std::string(
                    strerror(errno)) + "]");
        else
        {
        ...
        throw (std::runtime_error)
{
    ...
    if (ret < 0)
        throw std::runtime_error("epoll_ctl fail [" + std::string(strerror(
                errno)) + "]");
}
